        ...
        if (i % 8 == 7) {
            ...
            if (i == GetFreeInodeNum()) {
                if (vGetInodeBytemapValue(i) == 0) {
                    ...
                    printf(
                        "Testcase 1: Fail : InodeBytemap byte %d is not 0.\n",
                        GetFreeInodeNum());
                    return;
                }
            } else {
        ...
        if (i % 8 == 7) {
            for (j = 0; j < TOTAL_BLKPTR_NUM_PER_INDIRECT_BLOCK; j++) {
                ...
                if (j == GetIndirectBlockEntry(i, j)) {
                    if (j != vGetIndirectBlockEntry(i, j)) {
                        printf("Testcase 3: Fail : Indirect block %d, The "
                               "value of entry %d is %d.\n",
                               i, j, vGetIndirectBlockEntry(i, j));
                        return;
                    }
                } else {
                    printf("Testcase 3: Fail: Put/GetIndirectBlockEntry() is "
                           "incorrect!\n");
                    return;
                }
            }
            for (j = 0; j < TOTAL_BLKPTR_NUM_PER_INDIRECT_BLOCK; j++) {
                ...
                if (vGetIndirectBlockEntry(i, j) != INVALID_ENTRY) {
                    printf("Testcase 3: Fail : RemoveIndirectBlockEntry() is "
                           "incorrect!\n");
                    return;
                }
            }
        ...
        if (i % 8 == 7) {
            for (j = 0; j < TOTAL_DIRENTRY_NUM_PER_BLOCK; j++) {
                ...
                if (tmpDirEntry.inodeNum != dirEntry.inodeNum ||
                    ...
                    printf(
                        "Testcase 4: Fail: block %d, index %d is incorrect!\n",
                        i, j);
                    return;
                }
                ...
                if (tmpDirEntry.inodeNum != dirEntry.inodeNum ||
                    strcmp(tmpDirEntry.name, dirEntry.name) != 0) {
                    printf("Testcase 4: Fail: block %d, index %d, DirEntry "
                           "does not exist in disk!\n",
                           i, j);
                    return;
                }
            }
    ...
    for (i = 0; i < TOTAL_BLOCK_NUM; i++) {
        if (i % 8 == 7) {
            for (j = 0; j < TOTAL_DIRENTRY_NUM_PER_BLOCK; j++) {
                ...
                if (GetDirEntry(i, j, &tmpDirEntry) > 0) {
                    printf(
                        "Testcase 4: Fail: block %d, index %d is not removed\n",
                        i, j);
                    return;
                }
                ...
                if (tmpDirEntry.inodeNum != INVALID_ENTRY) {
                    ...
                    printf("Testcase 4: Fail: inode number of removed direntry "
                           "is valid.\n");
                    return;
                }
                ...
                if (tmpDirEntry.inodeNum != INVALID_ENTRY) {
                    printf("fourth tmp inode : %d \n", tmpDirEntry.inodeNum);
                    printf("Testcase 4: Fail: inode number of removed direntry "
                           "is valid.\n");
                    return;
                }
